...
use App\Models\User;
use App\Notifications\HospitalPasswordReset;
...
class HospitalManagement extends Controller
{
  public function hospitalManagement()
  {
    ...
    return view('content.laravel-example.hospital-management', [
      'totalUser' => $userCount,
      'verified' => $verified,
  ...
  public function index(Request $request)
  {
    ...
    // Initialize the user query builder with the "hospital" role constraint
    $usersQuery = User::whereHas('roles', function ($query) use ($hospitalRole) {
      $query->where('role_id', $hospitalRole->id);
    });
  ...
  public function store(Request $request)
  {
    ...
    if ($userID) {
      ...
      $users = User::updateOrCreate(
        ['id' => $userID],
        [
          'name' => $request->name, 'email' => $request->email, 'contact' => $request->contact, 'latitude' => $request->latitude,
      ...
      if (empty($userEmail)) {
        $user = User::updateOrCreate(
          ...
          [
            ...
            'contact' => $request->contact, 'latitude' => $request->latitude,
            'longitude' => $request->longitude

          ]
        );
        $user->assignRole('hospital', 'departmentmanage', 'doctormanage');
